...
void PmergeMe::insertionSort( void) {
    int initialSize = this->_argVector.size();
    
    std::cout << "INSERTION SORT FUNC" << std::endl;
    for (int i = 1; i < initialSize; i++)
    {
        int key = this->_argVector[i];
		std::cout << "key = " << key << std::endl;
        int j = i - 1;

        while (j >= 0 && this->_argVector[j] > key)
        {
			std::cout << "before swap : [j+1] : " << this->_argVector[j+1] << " ; [j] : " << this->_argVector[j] << std::endl;
            this->_argVector[j + 1] = this->_argVector[j];
            j--;
			std::cout << "after swap : [j+1] : " << this->_argVector[j+1] << " ; [j] : " << this->_argVector[j] << std::endl;

        }
		std::cout << "after loop : [j+1] : " << this->_argVector[j+1] << std::endl;
        this->_argVector[j + 1] = key;
		std::cout << "after after loop : [j+1] : " << this->_argVector[j+1] << std::endl;
		this->printVector();
    }
...
void PmergeMe::merge(int begin, int mid, int end ) {
	...
	for (int i = 0; i < n1; i++)
		L[i] = this->_argVector[begin + i];
	for (int j = 0; j < n2; j++)
		R[j] = this->_argVector[mid + 1 + j];
 ...
 // Until we reach either end of either L or R, pick larger among
  // elements L and M and place them in the correct position at A[p..r]
	while (i < n1 && j < n2)
	{
		if (L[i] <= R[j])
		{
			this->_argVector[k] = L[i];
			i++;
		}
		else
		{
			this->_argVector[k] = R[j];
			j++;
		}
		k++;
	}
